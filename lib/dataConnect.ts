export async function executeGraphQL(query: string, variables: any = {}) {
  ...
  console.log(`[DataConnect Debug] URL: ${dataConnectUrl}`);
  console.log(`[DataConnect Debug] Variables: ${JSON.stringify(variables)}`);
  ...
  console.log(`[DataConnect Debug] Status: ${response.status}`);
  const result = await response.json();
  console.log(`[DataConnect Debug] Response: ${JSON.stringify(result)}`);
